...
local icons, lsp, highlight, decorations = vim6.ui.icons, vim6.ui.lsp, vim6.highlight, vim6.ui.decorations
local api, fn, fs, fmt = vim.api, vim.fn, vim.fs, string.format
local P = vim6.ui.palette
...
---@param path string
---@return string
local function path_sep(path) return not vim6.empty(path) and path .. sep or path end
...
local function filename(ctx)
  ...
  local path = api.nvim_buf_get_name(buf)
  if vim6.empty(path) then return { fname = 'No Name' } end
  ...
  if vim6.empty(parent) then return { fname = fname } end
...
local function stl_file(ctx, minimal)
  ...
  local env_empty, dir_empty, parent_empty = vim6.empty(p.env), vim6.empty(p.dir), vim6.empty(p.parent)
...
local function diagnostic_info(context)
  ...
  local lsp_icons = vim6.ui.icons.lsp
  local result = {
  ...
  return vim6.fold(function(accum, item)
...
function vim6.ui.statusline.lsp_client_click()
...
local function stl_lsp_clients(ctx)
  ...
  if vim6.empty(clients) then return { { name = 'No LSP clients available', priority = 7 } } end
...
---@return string
function vim6.ui.statusline.render()
  ...
  local lsp_clients = vim6.map(function(client, index)
    return component(client.name, hls.client, {
      ...
      click = 'v:lua.vim6.ui.statusline.lsp_client_click',
...
local function adopt_window_highlights()
  ...
  if vim6.empty(curr_winhl) or not curr_winhl.StatusLine then return end

  for _, part in pairs(stl_winhl) do
    local name = part.hl(api.nvim_get_current_win())
    local hl = highlight.get(name)
    if not vim6.empty(hl) then return end
    highlight.set(
      name,
...
local set_stl_ft_icon_hls, reset_stl_ft_icon_hls = (function()
  ...
  return function(buf, ft)
    if vim6.empty(ft) then return end
...
-- set the statusline
vim.o.statusline = '%{%v:lua.vim6.ui.statusline.render()%}'
