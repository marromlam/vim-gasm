if not pde or not pde.ui.statusline.enable then return end
...
local icons, lsp, highlight, decorations =
  pde.ui.icons, pde.ui.lsp, pde.highlight, pde.ui.decorations
local api, fn, fs, fmt, strwidth =
  vim.api, vim.fn, vim.fs, string.format, vim.api.nvim_strwidth
local P, falsy = pde.ui.palette, pde.falsy
...
local function colors()
  ...
  --- pde italic or bold, this is because the underlying bold font is not necessarily
...
local set_filetype_icon_highlights, reset_filetype_icon_highlights = (function()
  ...
  return function(buf, ft)
    ...
    for _, data in pairs(hl_cache) do
      highlight.set(
        data.name,
        { fg = data.hl, bg = highlight.get(hls.statusline, 'bg') }
      )
    end
  end
end)()
...
--- directory of mine such pde my projects directory or my work directory
...
local function diagnostic_info(context)
  ...
  local lsp_icons = pde.ui.icons.lsp
  local result = {
  ...
  return pde.fold(function(accum, item)
...
function pde.ui.statusline.lsp_client_click()
...
---@return string
function pde.ui.statusline.render()
  ...
  local lsp_clients = pde.map(function(client)
    return {
      ...
      click = 'v:lua.pde.ui.statusline.lsp_client_click',
...
-- set the statusline
vim.o.statusline = '%{%v:lua.pde.ui.statusline.render()%}'

pde.augroup('CustomStatusline', {
  event = 'FocusGained',
