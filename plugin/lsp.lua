...
local function augroup_factory(bufnr, client, events)
  return function(feature, commands)
    ...
    if not provider or client.server_capabilities[provider] then
      events[name].group_id = vim6.augroup(fmt("LspCommands_%d_%s", bufnr, name), commands(provider))
...
local function setup_mappings(_, bufnr)
  ...
  map("n", "]c", function() vim.diagnostic.goto_prev({ float = true }) end, with_desc("lsp: go to prev diagnostic"))
  map("n", "[c", function() vim.diagnostic.goto_next({ float = true }) end, with_desc("lsp: go to next diagnostic"))
...
do
  ...
  local function smart_quickfix_diagnostics()
    ...
    if not vim6.is_vim_list_open() and id then
    ...
    id = id
      or api.nvim_create_autocmd("DiagnosticChanged", {
        callback = function()
          ...
          if not vim6.is_vim_list_open() or fn.getqflist({ title = 0 }).title ~= TITLE then return end
          diagnostic.setqflist({ open = false, title = TITLE })
          if #fn.getqflist() == 0 then vim6.toggle_qf_list() end
        end,
      })
  end
