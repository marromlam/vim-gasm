...
local icons = pde.ui.icons.lsp
local border = pde.ui.current.border
local augroup = pde.augroup
local command = pde.command
...
local function prepare_rename(data)
  ...
  for _, client in pairs(lsp.get_active_clients({ bufnr = bufnr })) do
    ...
    if not vim.tbl_get(client, rename_path) then
      return vim.notify(
        fmt('%s does not LSP file rename', client.name),
        'info',
        { title = 'LSP' }
      )
    end
...
local function show_related_locations(diag)
  ...
  for _, info in ipairs(related_info) do
    diag.message = ('%s\n%s(%d:%d)%s'):format(
      ...
      not pde.falsy(info.message) and (': %s'):format(info.message) or ''
    )
  end
  return diag
end
...
local function setup_mappings(client, bufnr)
  ...
  local mappings = {
    ...
    },
    {
      ...
      lsp.buf.format,
      desc = 'format buffer',
      capability = provider.FORMATTING,
      exclude = ts,
    },
    {
      'n',
    ...
    { 'n', 'gI', lsp.buf.incoming_calls, desc = 'show incoming calls' }, -- TODO: what provider is this?
    { 'n', 'gi', lsp.buf.implementation, desc = 'go to implementation' }, -- TODO: what provider is this?
    {
      'n',
      '<leader>gd',
...
local function setup_autocommands(client, buf)
  if not client.server_capabilities[provider.FORMATTING] then
    augroup(('LspFormatting%d'):format(buf), {
      ...
      command = function(args)
        if
          not vim.g.formatting_disabled and not vim.b[buf].formatting_disabled
        then
...
command(
  'LspFormat',
  function() lsp.buf.format({ bufnr = 0, async = false }) end
)
...
local function max_diagnostic(callback)
  return function(_, bufnr, diagnostics, opts)
    local max_severity_per_line = pde.fold(function(diag_map, d)
