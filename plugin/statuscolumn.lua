if not pde or not pde.ui.statuscolumn.enable then return end
...
local ui, sep, falsy = pde.ui, pde.ui.icons.separators, pde.falsy
...
local function extmark_signs(curbuf, lnum)
  ...
  local sns = pde.fold(function(acc, item)
...
--- cannot be placed in the same column pde other git signs which means they have to be manually
...
function ui.statuscolumn.render()
  ...
  local r1 = section:new(
    spacer(1),
    { { { nr(win, lnum, relnum, virtnum, line_count) } } },
    unpack(gitsign)
  )
  local r2 = section:new(
    { { { separator, 'LineNr' } }, after = '' },
    { { { fdm(lnum) } } }
  )
...
opt.statuscolumn = '%{%v:lua.pde.ui.statuscolumn.render()%}'

pde.augroup('StatusCol', {
