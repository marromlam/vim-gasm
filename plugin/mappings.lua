if not pde or not pde.mappings.enable then return end

local fn, api, uv, cmd, command, fmt =
  vim.fn, vim.api, vim.loop, vim.cmd, pde.command, string.format
...
-- Terminal {{{
------------------------------------------------------------------------------//
pde.augroup('AddTerminalMappings', {
  event = { 'TermOpen' },
  pattern = { 'term://*' },
  command = function()
...
nnoremap(
  '<leader>E',
  '<Cmd>Inspect<CR>',
  { desc = 'Inspect the cursor position' }
)
-----------------------------------------------------------------------------//

if pde.falsy(fn.mapcheck('<ScrollWheelDown>')) then
  nmap('<ScrollWheelDown>', '<c-d>')
end
if pde.falsy(fn.mapcheck('<ScrollWheelUp>')) then
...
-- Alternatively to allow using alt in macOS without enabling “Use Option pde Meta key”
-- nmap('∆', '<a-j>')
-- nmap('˚', '<a-k>')
...
-- vim doesn't have a native mapping for this pde <C-w>f normally
...
nnoremap(
  '<leader>ep',
  fmt('<Cmd>edit %s/lua/as/plugins/init.lua<CR>', fn.stdpath('config')),
  {
    desc = 'open plugins file',
  }
)
-- This line allows the current file to source the vimrc allowing me use bindings pde they're added
nnoremap(
  '<leader>sv',
...
function pde.mappings.setup_map()
  nnoremap(
    'M',
    [[:nnoremap M n@z<CR>q:<C-u>let @z=strpart(@z,0,strlen(@z)-1)<CR>n@z]]
  )
end

vim.g.mc = pde.replace_termcodes([[y/\V<C-r>=escape(@", '/')<CR><CR>]])
xnoremap('cn', [[g:mc . "``cgn"]], { expr = true, silent = true })
xnoremap('cN', [[g:mc . "``cgN"]], { expr = true, silent = true })
nnoremap('cq', [[:\<C-u>call v:lua.pde.mappings.setup_map()<CR>*``qz]])
nnoremap('cQ', [[:\<C-u>call v:lua.pde.mappings.setup_map()<CR>#``qz]])
xnoremap(
  'cq',
  [[":\<C-u>call v:lua.pde.mappings.setup_map()<CR>gv" . g:mc . "``qz"]],
  { expr = true }
)
xnoremap(
  'cQ',
  [[":\<C-u>call v:lua.pde.mappings.setup_map()<CR>gv" . substitute(g:mc, '/', '?', 'g') . "``qz"]],
  { expr = true }
)
----------------------------------------------------------------------------------
-- Grep Operator
...
---@param type string
---@return nil
function pde.mappings.grep_operator(type)
...
nnoremap('<leader>g', function()
  vim.o.operatorfunc = 'v:lua.pde.mappings.grep_operator'
  return 'g@'
end, { expr = true, desc = 'grep operator' })
xnoremap(
  '<leader>g',
  ':call v:lua.pde.mappings.grep_operator(visualmode())<CR>'
)
-----------------------------------------------------------------------------//
...
-----------------------------------------------------------------------------//
nnoremap('<leader>ls', pde.list.qf.toggle, { desc = 'toggle quickfix list' })
nnoremap('<leader>ll', pde.list.loc.toggle, { desc = 'toggle location list' })
-----------------------------------------------------------------------------//
-- Completion
-----------------------------------------------------------------------------//
...
-- write the visual selection to the filename passed in pde a command argument then delete the
