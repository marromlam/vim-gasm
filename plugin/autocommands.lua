...
local function stop_hl()
  ...
  api.nvim_feedkeys(pde.replace_termcodes('<Plug>(StopHL)'), 'm', false)
end
...
pde.augroup('VimrcIncSearchHighlight', {
  event = { 'CursorMoved' },
...
local smart_close_filetypes = pde.p_table({
  ['qf'] = true,
  ['log'] = true,
  ['help'] = true,
  ['query'] = true,
...
local smart_close_buftypes = pde.p_table({
  ['nofile'] = true,
})
...
pde.augroup('TerminalAutocommands', {
  ...
  command = function(args)
    ...
    if pde.falsy(v.event.status) and pde.falsy(vim.bo[args.buf].ft) then
