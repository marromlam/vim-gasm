...
local icons = core.style.icons.lsp
...
local function augroup_factory(bufnr, client, events)
    return function(feature, commands)
        ...
        if not provider or client.server_capabilities[provider] then
            events[name].group_id = core.augroup(fmt('LspCommands_%d_%s', bufnr, name), commands(provider))
...
local function setup_mappings(_, bufnr)
    ...
    core.nnoremap('<leader>rf', format, with_desc('lsp: format buffer'))
    core.nnoremap('gd', lsp.buf.definition, with_desc('lsp: definition'))
    core.nnoremap('gr', lsp.buf.references, with_desc('lsp: references'))
    core.nnoremap('K', lsp.buf.hover, with_desc('lsp: hover'))
    core.nnoremap('gI', lsp.buf.incoming_calls, with_desc('lsp: incoming calls'))
    core.nnoremap('gi', lsp.buf.implementation, with_desc('lsp: implementation'))
    core.nnoremap('<leader>gd', lsp.buf.type_definition, with_desc('lsp: go to type definition'))
    core.nnoremap('<leader>cl', lsp.codelens.run, with_desc('lsp: run code lens'))
    core.nnoremap('<leader>rn', lsp.buf.rename, with_desc('lsp: rename'))
end
...
-- Commands
-----------------------------------------------------------------------------//
local command = core.new_command
...
do
    ...
    local function smart_quickfix_diagnostics()
        ...
        if not core.is_vim_list_open() and id then
        ...
        id = id
            or api.nvim_create_autocmd('DiagnosticChanged', {
                callback = function()
                    ...
                    if not core.is_vim_list_open() or fn.getqflist({ title = 0 }).title ~= TITLE then return end
                    diagnostic.setqflist({ open = false, title = TITLE })
                    if #fn.getqflist() == 0 then core.toggle_qf_list() end
                end,
            })
    end

    command('LspDiagnostics', smart_quickfix_diagnostics)
    core.nnoremap('<leader>ll', '<Cmd>LspDiagnostics<CR>', 'toggle quickfix diagnostics')
end
