local api, notify, fmt, augroup = vim.api, vim.notify, string.format, mrl.augroup
...
local function get_hl_as_hex(opts, ns)
  ...
  if pcall(api.nvim_get_hl(ns, opts)) then
    local hl = api.nvim_get_hl(ns, opts)
  else
  hl = {bg="#000000", fg="#ff0000"}
end
...
local function get(group, attribute, fallback)
  fallback = fallback or  {bg="#000000", fg="#ff0000"}
...
local function all(hls, namespace)
  mrl.foreach(function(hl) set(namespace or 0, next(hl)) end, hls)
end
