...
local function is_list_open(list_type)
  return mrl.find(function(win) return not mrl.falsy(win[list_type]) end, fn.getwininfo()) ~= nil
end
...
function mrl.truncate(str, max_len)
  ...
  return api.nvim_strwidth(str) > max_len and str:sub(1, max_len) .. mrl.ui.icons.misc.ellipsis or str
end
...
function mrl.filetype_settings(map)
  local commands = mrl.map(function(settings, ft)
    ...
    return {
      ...
      command = function(args)
        mrl.foreach(function(value, scope)
          if scope == 'opt' then scope = 'opt_local' end
          if scope == 'mappings' then return apply_ft_mappings(value, args.buf) end
          if scope == 'plugins' then return mrl.ftplugin_conf(value) end
          if type(value) ~= 'table' and vim.is_callable(value) then return value(args) end
          mrl.foreach(function(setting, option) vim[scope][option] = setting end, value)
        end, settings)
      end,
    }
  end, map)
  mrl.augroup('filetype-settings', unpack(commands))
end
...
---@param str string
---@return string
function mrl.replace_termcodes(str) return api.nvim_replace_termcodes(str, true, true, true) end
