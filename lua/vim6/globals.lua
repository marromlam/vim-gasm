...
---@param accum T?
---@return T
function vim6.fold(callback, list, accum)
  accum = accum or {}
...
---@param list T[]
---@return T[]
function vim6.map(callback, list)
  return vim6.fold(function(accum, v, k)
...
---@param list T[]
function vim6.foreach(callback, list)
  for k, v in pairs(list) do
    callback(v, k)
  end
end
...
---@param haystack T[]
---@return T
function vim6.find(matcher, haystack)
  local found
...
function vim6.installed_plugins()
...
---@return boolean
function vim6.is_vim_list_open()
...
function vim6.adjust_split_height(minheight, maxheight)
...
local function toggle_list(list_type)
  ...
  local is_open = vim6.is_vim_list_open()
...
function vim6.toggle_qf_list() toggle_list("quickfix") end
function vim6.toggle_loc_list() toggle_list("location") end

---@param str string
---@param max_len integer
---@return string
function vim6.truncate(str, max_len)
  assert(str and max_len, "string and max_len must be provided")
  return api.nvim_strwidth(str) > max_len and str:sub(1, max_len) .. vim6.ui.icons.misc.ellipsis or str
end

---Determine if a value of any type is empty
---@param item any
---@return boolean?
function vim6.empty(item)
...
---@param opts table?
---@return boolean, any
function vim6.require(module, opts)
...
function vim6.wrap_err(msg, func, ...)
  local args = { ... }
...
function vim6.ftplugin_conf(configs)
  if type(configs) ~= "table" then return end
  for name, callback in pairs(configs) do
    local info = debug.getinfo(1, "S")
    local ok, plugin = vim6.require(name, { message = fmt("In file: %s", info.source) })
    if ok then callback(plugin) end
  end
end

local LATEST_NIGHTLY_MINOR = 9
function vim6.nightly() return vim.version().minor >= LATEST_NIGHTLY_MINOR end

---Reload lua modules
---@param path string
---@param recursive boolean
function vim6.invalidate(path, recursive)
  if recursive then
...
---@return unknown
function vim6.version(major, minor, _)
...
--- Validate the keys passed to vim6.augroup are valid
---@param name string
---@param cmd Autocommand
local function validate_autocmd(name, cmd)
  local keys = { "event", "buffer", "pattern", "desc", "command", "group", "once", "nested" }
  local incorrect = vim6.fold(function(accum, _, key)
...
---@param opts table?
function vim6.command(name, rhs, opts)
  opts = opts or {}
...
---@param e string
---@return boolean
function vim6.executable(e) return fn.executable(e) > 0 end

---A terser proxy for `nvim_replace_termcodes`
---@param str string
---@return string
function vim6.replace_termcodes(str) return api.nvim_replace_termcodes(str, true, true, true) end

---check if a certain feature/version/commit exists in nvim
---@param feature string
---@return boolean
function vim6.has(feature) return fn.has(feature) > 0 end
