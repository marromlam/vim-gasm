...
function M.component(item, hl, opts)
  ...
  if opts.max_size and item and #item >= opts.max_size then item = item:sub(1, opts.max_size - 1) .. "…" end
...
local function sum_lengths(list)
  return vim6.fold(function(acc, item) return acc + (item.length or 0) end, list, 0)
end
