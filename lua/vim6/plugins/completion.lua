local highlight, t, fmt = vim6.highlight, vim6.replace_termcodes, string.format
local api, fn = vim.api, vim.fn
local border = vim6.ui.current.border
...
return {
  {
    ...
    dependencies = {
      ...
      { "hrsh7th/cmp-cmdline", enabled = not vim6.nightly() },
      { "dmitmel/cmp-cmdline-history", enabled = not vim6.nightly() },
      { "hrsh7th/cmp-nvim-lsp-document-symbol", enabled = not vim6.nightly() },
      {
        "f3fora/cmp-spell",
    ...
    config = function()
      ...
      local kind_hls = vim6.ui.lsp.highlights
      local ellipsis = vim6.ui.icons.misc.ellipsis
      ...
      -- stylua: ignore
      highlight.plugin('Cmp', vim6.fold(function(accum, value, key)
      ...
      cmp.setup({
        ...
        mapping = {
          ["<C-a>"] = cmp.mapping(function(_) api.nvim_feedkeys(fn["copilot#Accept"](t("<Tab>")), "n", true) end),
        ...
        formatting = {
          ...
          format = function(entry, vim_item)
            ...
            vim_item.kind = fmt("%s %s", vim6.ui.current.lsp_icons[vim_item.kind], vim_item.kind)
            vim_item.menu = ({
              nvim_lsp = "[LSP]",
              nvim_lua = "[Lua]",
              emoji = "[E]",
