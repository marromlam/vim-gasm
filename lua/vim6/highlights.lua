...
---@return string
function vim6.highlight.alter_color(color, percent)
...
function vim6.highlight.get(group, attribute, fallback)
...
---@return ErrorMsg[]?
function vim6.highlight.set(namespace, name, opts)
  ...
  for attr, value in pairs(opts) do
    if type(value) == "table" and value.from then
      local new_attr, err = vim6.highlight.get(value.from, value.attr or attr)
      if value.alter then new_attr = vim6.highlight.alter_color(new_attr, value.alter) end
...
function vim6.highlight.set_winhl(name, win_id, hls)
  local namespace = api.nvim_create_namespace(name)
  vim6.highlight.all(hls, namespace)
  api.nvim_win_set_hl_ns(win_id, namespace)
end

function vim6.highlight.clear(name)
...
function vim6.highlight.all(hls, namespace)
  local errors = vim6.fold(function(errors, hl)
    local curr_errors = vim6.highlight.set(namespace or 0, next(hl))
    if curr_errors then vim.list_extend(errors, curr_errors) end
    return errors
  end, hls)
  if #errors > 0 then notify(vim6.fold(function(acc, err) return acc .. "\n" .. err.msg end, errors, ""), "error") end
end
...
function vim6.highlight.plugin(name, opts)
  ...
  vim6.highlight.all(opts)
  vim6.augroup(fmt("%sHighlightOverrides", name), {
    event = "ColorScheme",
    command = function()
      -- Defer resetting these highlights to ensure they apply after other overrides
      vim.defer_fn(function() vim6.highlight.all(opts) end, 1)
    end,
  })
end
