...
function M.get(group, attribute, fallback)
  ...
  vim.schedule(
    function() vim.notify(fmt("%s's %s does not exist", group, attr), 'error') end
  )
  return 'NONE'
end
...
function M.win_hl.adopt(win_id, target, name, fallback)
  ...
  local hl = core.find(
    function(part) return part:match(target) end,
    vim.split(win_hl, ',')
  )
...
local function general_overrides()
  M.all({
    ...
    { DiagnosticWarning = { foreground = '#FF0000' } },
...
local function colorscheme_overrides()
  local overrides = {
    ...
    ['horizon'] = {
      ...
      {
        VisibleTab = {
          background = { from = 'Normal', alter = 40 },
          bold = true,
        },
      },
