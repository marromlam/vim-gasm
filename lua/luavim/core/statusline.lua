...
local function sum_lengths(list)
    return core.fold(function(acc, item) return acc + (item.length or 0) end, list, 0)
end
...
local function filename(ctx, modifier)
    ...
    if not fname or core.empty(fname) then return '', '', 'No Name' end
...
local function filetype(ctx, opts)
    ...
    local icons_loaded, devicons = core.require('nvim-web-devicons')
...
function M.file(ctx, minimal)
    ...
    local dir_empty, parent_empty = core.empty(directory), core.empty(parent)
...
function M.diagnostic_info(context)
    ...
    local lsp = core.style.icons.lsp
    local result = {
    ...
    result = core.fold(function(accum, item)
