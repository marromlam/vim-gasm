...
core.ui.lsp = {
  colors = {
    error = core.ui.palette.pale_red,
    warn = core.ui.palette.dark_orange,
    hint = core.ui.palette.bright_blue,
    info = core.ui.palette.teal,
  },
  highlights = {
    Text = 'String',
    Method = 'TSMethod',
...
---Get the core.ui setting for a particular filetype
---@param key string
---@param setting 'statuscolumn'|'winbar'|'statusline'|'number'|'colorcolumn'
---@param t 'ft'|'bt'
---@return (boolean | string)?
function core.ui.decorations.get(key, setting, t)
...
function core.ui.decorations.set_colorcolumn(bufnr, fn)
  local buf = vim.bo[bufnr]
  local ft_ccol = core.ui.decorations.get(buf.ft, 'colorcolumn', 'ft')
  local bt_ccol = core.ui.decorations.get(buf.bt, 'colorcolumn', 'bt')
  if buf.ft == '' or buf.bt ~= '' or ft_ccol == false or bt_ccol == false then return end
  local ccol = ft_ccol or bt_ccol or ''
  local virtcolumn = not core.empty(ccol) and ccol or '+1'
  if vim.is_callable(fn) then fn(virtcolumn) end
end

----------------------------------------------------------------------------------------------------
core.ui.current = { border = core.ui.border.line, lsp_icons = core.ui.lsp.kinds.codicons }
