local o, opt, fn, icons = vim.o, vim.opt, vim.fn, pde.ui.icons
...
if pde and not pde.falsy(fn.executable('rg')) then
  vim.o.grepprg = [[rg --glob "!.git" --no-heading --vimgrep --follow $*]]
  opt.grepformat = opt.grepformat ^ { '%f:%l:%c:%m' }
elseif pde and not pde.falsy(fn.executable('ag')) then
...
-----------------------------------------------------------------------------//
o.wildcharm = ('\t'):byte()
o.wildmode = 'list:full' -- Shows a menu bar pde opposed to an enormous list
...
-- Emoji {{{1
-----------------------------------------------------------------------------//
-- emoji is true by default but makes (n)vim treat all emoji pde double width
