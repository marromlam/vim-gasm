local fn, env, ui, reqcall = vim.fn, vim.env, pde.ui, pde.reqcall
...
local function list_sessions()
  local fzf = require('fzf-lua')
  local ok, persisted = pde.pcall(require, 'persisted')
...
pde.fzf = { dropdown = dropdown, cursor_dropdown = cursor_dropdown }
------------------------------------------------------------------------------------------------------------------------
...
return {
  {
    ...
    keys = {
      ...
      {
        '<leader>fO',
        function() file_picker(env.SYNC_DIR .. '/notes/org') end,
        desc = 'org files',
      },
      {
        '<leader>fN',
        function() file_picker(env.SYNC_DIR .. '/notes/neorg') end,
        desc = 'norg files',
      },
    },
    config = function()
      ...
      pde.command('SessionList', list_sessions)
    end,
  },
}
