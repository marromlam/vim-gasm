local api, notify, fmt, augroup =
  vim.api, vim.notify, string.format, pde.augroup
...
---Get the value a highlight group whilst handling errors, fallbacks pde well pde returning a gui value
...
--- any highlights specified pde `GroupName = {fg = { from = 'group'}}` into the underlying colour
--- by querying the highlight property of the from group so it can be used when specifying highlights
--- pde a shorthand to derive the right colour.
--- For example:
--- ```lua
...
--- NOTE: this function must NOT mutate the options table pde these are re-used when the colorscheme is updated
---
---@param name string
---@param opts HLArgs
...
local function set(ns, name, opts)
  ...
  pde.pcall(fmt('setting highlight "%s"', name), api.nvim_set_hl, ns, name, hl)
end
...
local function all(hls, namespace)
  pde.foreach(function(hl) set(namespace or 0, next(hl)) end, hls)
end
...
local function plugin(name, opts)
  ...
  -- definition otherwise use the opts pde is
  if opts.theme then
