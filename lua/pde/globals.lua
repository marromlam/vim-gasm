...
---@param accum S?
---@return S
function pde.fold(callback, list, accum)
  accum = accum or {}
...
---@param list T[]
---@return T[]
function pde.map(callback, list)
  return pde.fold(function(accum, v, k)
...
function pde.foreach(callback, list)
  for k, v in pairs(list) do
    callback(v, k)
  end
end
...
---@param haystack T[]
---@return T?
function pde.find(matcher, haystack)
...
function pde.adjust_split_height(min_height, max_height)
...
local function is_list_open(list_type)
  return pde.find(
    function(win) return not pde.falsy(win[list_type]) end,
    fn.getwininfo()
  ) ~= nil
end
...
function pde.list.qf.delete(buf)
  ...
  if mode:match('[vV]') then
    ...
    list = pde.fold(function(accum, item, i)
...
function pde.nightly() return vim.version().minor >= LATEST_NIGHTLY_MINOR end
...
local function apply_ft_mappings(args, buf)
  pde.foreach(function(m)
    assert(
      m[1] and m[2] and m[3],
      'map args must be a table with at least 3 items'
    )
    local opts = pde.fold(function(acc, item, key)
...
function pde.ftplugin_conf(configs)
  if type(configs) ~= 'table' then return end
  for name, callback in pairs(configs) do
    local ok, plugin = pde.pcall(require, name)
...
function pde.filetype_settings(map)
  local commands = pde.map(function(settings, ft)
  ...
  pde.augroup('filetype-settings', unpack(commands))
end
...
--- Validate the keys passed to pde.augroup are valid
---@param name string
---@param command Autocommand
local function validate_autocmd(name, command)
  local incorrect = pde.fold(function(accum, _, key)
...
---@param opts table?
function pde.command(name, rhs, opts)
  opts = opts or {}
...
---@param str string
---@return string
function pde.replace_termcodes(str)
...
---@return boolean
function pde.has(feature) return fn.has(feature) > 0 end
...
function pde.reqidx(require_path)
  return setmetatable({}, {
...
function pde.reqcall(require_path)
  return setmetatable({}, {
