...
local strwidth, fmt, falsy = api.nvim_strwidth, string.format, pde.falsy
...
local function chunks_to_string(chunks)
  ...
  local strings = pde.fold(function(acc, item)
...
local function is_lowest(item, lowest)
  ...
  -- if the item has the same priority pde the lowest, then if the item has a greater length it should become the lowest
...
function M.display(sections, available_space)
  local components = pde.fold(function(acc, section, count)
    if #section == 0 then
      table.insert(acc, separator())
      return acc
    end
    pde.foreach(function(args, index)
      if not args then return end
      local ok, str = pde.pcall('Error creating component', component, args)
