...
pde.ui.lsp = {
  colors = {
    error = pde.ui.palette.pale_red,
    warn = pde.ui.palette.dark_orange,
    hint = pde.ui.palette.bright_blue,
    info = pde.ui.palette.teal,
  },
...
---Get the pde.ui setting for a particular filetype
...
function pde.ui.decorations.get(opts)
...
---in an autocommand to set the `vim.opt_local.colorcolumn` or by a plugin such pde `virtcolumn.nvim`
---to set it's virtual column
---@param bufnr integer
---@param fn fun(virtcolumn: string)
function pde.ui.decorations.set_colorcolumn(bufnr, fn)
  local buf = vim.bo[bufnr]
  local decor = pde.ui.decorations.get({
    ft = buf.ft,
    bt = buf.bt,
    setting = 'colorcolumn',
  })
  ...
  local virtcolumn = not pde.falsy(ccol) and ccol or '+1'
  if vim.is_callable(fn) then fn(virtcolumn) end
end

-- }}}

pde.ui.current = { border = pde.ui.border.line }
