local fn, env, ui, reqcall = vim.fn, vim.env, mrl.ui, mrl.reqcall
    ...
    dropdown({
      ...
      actions = {
        ["default"] = function(selected)
          local session = vim.tbl_filter(function(s)
        ...
        ["ctrl-d"] = {
          function(selected)
            local session = vim.iter(sessions):find(function(s)
              return s.name == selected[1]
            end)
            if not session then
              return
...
return {
  {
    ...
    keys = {
      ...
      },
      {
        "<leader>fO",
        function()
          file_picker(env.SYNC_DIR .. "/notes/org")
        end,
        desc = "org files",
      },
      {
        "<leader>fN",
        function()
          file_picker(env.SYNC_DIR .. "/notes/neorg")
        end,
        desc = "norg files",
      },
    },
